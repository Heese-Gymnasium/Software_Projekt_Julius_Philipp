...
func _finish_move(idx, trgt_coords):
	...
	if(trgt_x <= get_parent().get_parent().Map_x && trgt_y <= get_parent().get_parent().Map_y && trgt_x >= 0 && trgt_y >= 0):
		#print(get_parent().get_parent().Map[trgt_y][trgt_x])
		#get_parent().get_parent().print_map()   						#debug messages
		if(get_parent().get_parent().Map[trgt_y][trgt_x][0] == 0):
			get_parent().get_parent().Map = [trgt_y, trgt_x, [idx, -7]]
			get_parent().get_parent().Map = [y, x, [0, -7]]
